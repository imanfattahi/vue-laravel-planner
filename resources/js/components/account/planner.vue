<template>
    <div class="container">
        ...
        <modal v-if="planModal" :large="true" :hcolor="'rgb(255, 205, 46)'">
            <h5 slot="title">
                {{ this.edit === true ? 'Edit Plan' : 'Set New Plan' }}
            </h5>
            <div slot="body">
                <div class="form-group">
                    <label for="title">Select Date:</label>
                    <date-picker min="window.moment().format('YYYY-MM-DD HH:mm')" v-model="plan.start" locale="en,fa" format="YYYY-MM-DD HH:mm" type="datetime" :jumpMinute="60" :roundMinute="true"/>
                </div>

                <div class="form-group">
                    <label for="title">Title:</label>
                    <input v-model="plan.title" type="text" class="form-control" id="title" placeholder="Enter a short title">
                </div>
            ...
            <button type="button" class="btn btn-success float-right" @click="setPlan()">
                Set Plan
            </button>
            <button v-if="edit" type="button" class="btn btn-danger float-right" @click="deletePlan()" style="margin-right: 25px">
                Delete
            </button>
            <button @click="planModal = false" type="button" class="btn btn-info">
                Cancel
            </button>
            </div>
        </modal>
...
export default {
    ...
    beforeMount() {
        ...
        setPlan: function () {
            ...
            self.$http.post('/account/planner/set', {plans: self.plans}).then(function (response) {
                if (response.status = 200) {
        ...
        showDetails: function (plan, e) {
            ...
            var PLAN = self.plans.filter(function (x) {
                return (x.start == plan.start) && (x.end == plan.end) && (x.title == plan.title)
            })
            if (PLAN.length) {
                PLAN = PLAN[0]
